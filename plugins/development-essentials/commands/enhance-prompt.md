...
Reply with ONLY the rewritten instruction itself — no preamble, no explanation, no wrapper tags, no markers. Output must start directly with the first word of the enhanced prompt and end with its last word, so it can be `/copy`'d verbatim as-is.
